  ...
  return (
    <>
      <Modal isOpen={showDesc} onClose={() => setShowDesc(false)}>
        <Styled.BioBox>
          <h3>
            {t("cards:biography_for")} <Text.green>{artist.name}</Text.green>
          </h3>
          <hr />
...
function LastStats({ d }: { d?: ILastFMArtist }) {
  ...
  return (
    <div>
      <h6>
        <Text.pGreen>{d?.plays}</Text.pGreen> {t("cards:lastfm-plays")}
      </h6>
      <h6>
        <Text.pGreen>{d?.listeners}</Text.pGreen> {t("cards:lastfm-listeners")}
      </h6>
    </div>
  );
}

...
const bulk = async (
    ...
    const [database_res, error] = await backendDB.getTracksDetails(body.tracks.map(t => t.id))
    ...
    if (error || !database_res) {
        ...
        trackDetails = database_res[0].map(t => {
            return {
                id: t.PK,
                genres: t.genres?.map(g => ({label: g.label, confidence: g.confidence})) || [],
                moods: [t.happy, t.sad, t.acoustic, t.aggressive, t.relaxed, t.electronic],
    ...
    if (failures >0 && failures == chunks.length) {
        return res.status(400).json({error: `Track analysis for ${body.tracks.length} tracks failed`});
    }
